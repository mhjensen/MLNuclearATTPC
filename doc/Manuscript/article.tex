...
As an alternative to relying on a pre-trained model, we also consider the MIXAE algorithm {\color{cyan} [Ryan: citation]}, a bespoke end-to-end clustering model which is specifically trained on the AT-TPC data.

The MIXAE model comprises several autoencoders, each of which corresponds to a cluster. The number of autoencoders, and thus the number of clusters, has to be determined beforehand. Each autoencoder constructs a latent representation of a given example. Those representations are used as inputs to an auxiliary network which assigns probabilities the clusters, indicating the likelihood that the given example belongs to each cluster. Examples are then assigned to the cluster with the highest probability. {\color{cyan} [Ryan: A description of the objective functions may or may not be worth including here, but I don't know what they are.]} The architecture is portrayed in Figure \ref{fig:mixae}, tapered boxes denote a direction of compression in the network components. 
...
\noindent {\color{cyan} [Ryan: Still needs a concluding sentence or two here.]} For a more formal consideration of these heuristics and the assumptions made on the data by this model see \citet{Zhang}.
