...
In addition, the selection task produced a binary result only, either a ``good" or``bad" fit to the event of interest. A ``bad'' fit is then assumed to be a different event type, and is removed from the analysis. %, if only because of the presence of carbon in the target (from the isobutane gas).
...
The architecture is portrayed in Figure \ref{fig:mixae}, wherein tapered boxes denote a direction of compression in the network components. In the figure each encoder and decoder pair constitute an autoencoder. The assignment of the cluster belonging of a sample, $\mathbf{\hat{x}}$, is taken to be the index of the maximal element in the vector $\mathbf{p}$ as shown in the right-most part of the figure. Finally, the model is trained end-to-end with backpropagation \cite{Linnainmaa1976}, as implemented in the machine learning library TensorFlow \cite{tensorflow}.
