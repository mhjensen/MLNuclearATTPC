...
We illustrate two filtered events in figure \ref{fig:samples_filtered}. These are the same events as shown in figure \ref{fig:samples}, but with the Hough transform and nearest neighbours filtering applied. 
...
To further investigate the clusters presented in the matrix in figures \ref{fig:clster_confmati_filt} and \ref{fig:clster_confmati_raw}, we study examples from the proton samples belonging to different clusters for the filtered and full data.
...
\begin{table}[H]
\centering 
\caption[MIXAE clustering performance]{MIXAE clustering performance on the ${}^{46} Ar$ experimental data. In contrast with the VGG-16 + $K$-means approach we observe  significant variations in performance. {\color{blue}, however, the KMeans++ algorithm still clusters by reaction type, as evidenced when initializing $10$ random centroid seeds, and selecting the most separated clusters without supervision. The mean and standard deviations come from $N = 1000$ runs.  }}\label{tab:clstr_mixae}
\input{./plots/mixae_clustering.tex}
\end{table}

As with the VGG16 + $K$-Means approach we wish to further investigate the clustering results. Taking the best performing MIXAE model on filtered and raw data we tabulate the clusters against their labels. These tables are present in figures \ref{fig:mixae_confmat_filtered} and \ref{fig:mixae_confmat_raw} for filtered and raw data, respectively.
