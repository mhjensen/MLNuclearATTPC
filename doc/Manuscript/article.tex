...
\title{Machine Learning Approaches  for Classification of Events in Active-Target Time Projection Chamber Experiments}
...
Additionally, we log-transform and normalize the data to the interval $[0, 1]$ for each point {\color{blue} are you sure we log transformed? were you given peaks data or the 2D projections? -MPK}. The final representation for each event is a $128 \times 128$ matrix with each element ranging from $0$ to $1$.
...
The MIXAE model comprises several autoencoders, each of which corresponds to a cluster. The number of autoencoders, and thus the number of clusters, has to be determined beforehand. Each autoencoder constructs a latent representation of a given example. Those representations are used as inputs to an auxiliary network which assigns probabilities to the clusters, indicating the likelihood that the given example belongs to each cluster. Examples are then assigned to the cluster with the highest probability. The models' objective can then be de-composed in three parts:
\begin{enumerate}
    \item Each batch should contain different classes
    \item Each predicted cluster assignment should be confident. 
    \item The reconstructions should be good. But as they are weighted by the cluster assignment, the importance of the reconstruction is proportional to the confidence in the cluster predictions.
\end{enumerate}

{\color{cyan} [Ryan: A description of the objective functions may or may not be worth including here, but I don't know what they are.]} The architecture is portrayed in Figure \ref{fig:mixae}, wherein tapered boxes denote a direction of compression in the network components. 
...
\noindent {\color{cyan} [Ryan: Still needs a concluding sentence or two here.]} For a more formal consideration of these heuristics, the model objective aand the assumptions made on the data by this model see \citet{Zhang}.
