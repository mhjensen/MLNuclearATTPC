...
\begin{table}
\centering
\caption{Descriptions of number of events in the data. In principle we can simulate an arbitrary amount of data, but the following quantities were shown to be sufficient for these studies.}\label{tab:datasets}
\begin{tabular}{lccc}
\toprule
...
The AT-TPC data we studied for this work was originally recorded as charge time-series for each pad on the micromegas {\color{green} We haven't defined what micromegas is}.
...
We repeat this analysis using a PCA dimensionality reduction {\color{green}(please define)} on the latent space of the VGG16 model. This is done to estimate to what degree the class separating information is encoded in the entirety of the latent space, or in some select regions. The results from the PCA analysis were virtually identical to the results sans the PCA, and so we omit them for brevity. 
